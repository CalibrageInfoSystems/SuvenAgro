...
public class AreaWaterTypeAdapter extends RecyclerView.Adapter<AreaWaterTypeAdapter.MyHolder> {
    ...
    public void onBindViewHolder(AreaWaterTypeAdapter.MyHolder holder, final int position) {
        ...
        holder.dischargeTxt.setText(String.format("%.2f", mWaterTypeModelList.get(position).getWaterdischargecapacity()));
//        holder.dischargeTxt.setText(""+mWaterTypeModelList.get(position).getWaterdischargecapacity());
        ...
        if (null != mWaterTypeModelList.get(position)) {
            ...
            holder.canalTxtLay.setVisibility(mWaterTypeModelList.get(position).getCanalwater() != 0 ? View.VISIBLE : View.GONE);
        }
        if (mWaterTypeModelList.get(position).getSourceofwaterid() == 175) {
holder.editIcon.setVisibility(View.GONE);
        } else {
holder.editIcon.setVisibility(View.VISIBLE);
 // set click listener only if visible
        }
